...
m_result(index,1).Q{1} =  [eye(4)/10000,zeros(4,2);zeros(2,4),zeros(2,2)];   %运动噪声小一点
m_result(index,1).R{1} = [eye(2),zeros(2,2);zeros(2,2),[0.0271650346223203,0;0,0.0000429199642412428]];     %观测噪声任取
...
for i = 2:useful_num
    %%  预测
       ...
       0  (Delta_u^2)*(Delta_time^2)/2  0  (Delta_u^2)*(Delta_time)];
        ...
        P_m_minus = A_expend *  m_result(index,1).P{i-1}(1:6,1:6) * A_expend' + diag([diag(m_result(index,1).Q{i-1})]);
     %% Rk更新
     % 获得观测误差
