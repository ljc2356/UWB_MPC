function [] = main_n_Anglefunction(files_name,AngleFormationIndex)
%% 读取数据
% clear all;close all;
folder = './ml_data/20210413_indoor/';
%  files_name = 'move_08.json';
...
%% 生成角度遍历数据
Formation_mat = GenAngleFormation();
%% 定位多径位置
% figure(1);
% for i = 1:8
...
%     i
    for antenna_num = 3:8
        index = antenna_num - 2 ;
        Formation_num = length(Formation_mat{antenna_num}(:,1));
        for j = 1:Formation_num
            antenna_index = Formation_mat{antenna_num}(j,:);
            ReadTime(After_records,index,i);
             ...
             result(index,1).los_d.data{1,j}(i,1) = D_Phi(1);
             result(index,1).los_phi.data{1,j}(i,1) = D_Phi(2);
             if (mpc_index(i,1) == 0) % 如果没有多径，则在多径的数据栏上增加标识: 多径的距离是 0 通过检测多径距离即可判断是否存在多径
                 result(index,1).mpc_d.data{1,j}(i,1) = 0;
                 result(index,1).mpc_phi.data{1,j}(i,1) = 0;
             else
    ...
    %                  [D_Phi,~] = mpc_analyse_form_Runtime(After_records(i,1),temp_mpc_index(mpc_th),los_path,antenna_index,sec_cali);
                     result(index,1).mpc_d.data{1,j}(i,mpc_th) = D_Phi(1);
                     result(index,1).mpc_phi.data{1,j}(i,mpc_th) = D_Phi(2);
                 end
             end
        end
    end
...
for antenna_num = 3:8
    ...
    for j = 1:Formation_num
        antenna_index = Formation_mat{antenna_num}(j,:);
         ...
         result(index,1).los_d.data{2,j} = antenna_index;
         result(index,1).los_phi.data{2,j} = antenna_index;
         if (mpc_index(i,1) == 0) % 如果没有多径，则在多径的数据栏上增加标识: 多径的距离是 0 通过检测多径距离即可判断是否存在多径
             result(index,1).mpc_d.data{2,j} = antenna_index;
             result(index,1).mpc_phi.data{2,j} = antenna_index;
         else
             temp_mpc_index = mpc_index(i,find(mpc_index(i,:)));
             for mpc_th = 1:length(temp_mpc_index)  % 如果是空的，则不会填写
                 result(index,1).mpc_d.data{2,j} = antenna_index;
...
data_folder = './data/20210413_indoor_angle/';
