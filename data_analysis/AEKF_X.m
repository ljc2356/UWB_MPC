...
%% AEKF 自适应 故障检测 卡尔曼滤波核心算法框架
% AEKF_Result
% 滤波信息存储结构体，存储了历史上所有的状态m、状态估计协方差矩阵P、观测噪声协方差矩阵R、运动噪声协方差矩阵Q，运动噪声残差w_flat,观测噪声残差e_flat，故障指示向量error_index
...
%观测值预测     
    zk_hat = hx(m_minus);
    Hxk = JHx(m_minus);
    ek = Observation - zk_hat;  %预测观测与实际观测残差
...
% 开始检验是否是正确的估计值
    lambda_k = ek' * Sk^(-1) * ek;
    if (lambda_k<= TD)   % 若检测出观测值与估计值正确 则输出当前计算结果
        AEKF_Result.R{i} = Rk;
    ...
    else  %  若检测出观测量有误 状态与状态协方差采用预测值，噪声模型沿用上一时刻值
